...
namespace RonaldoPenalty
{
	public class PenaltyGoalkeeperAI : MonoBehaviour
	{
		...
		[SerializeField]
		private bool changeSpeedByRound = false;
		...
		public void SetupRound(int round)
		{
			...
			{
				currentSpeed = baseSpeed + (float)(round - 1) * 0.5f;
			}
			else
			{
				currentSpeed = baseSpeed;
		...
		private void Update()
		{
			if (isPatrolling && !(leftPost == null) && !(rightPost == null))
			{
				t += (float)direction * currentSpeed * Time.deltaTime;
				if (t >= 1f)
				{
					t = 1f;
					direction = -1;
				...
				base.transform.position = Vector3.Lerp(leftPost.position, rightPost.position, smoothT);
			}
		}
